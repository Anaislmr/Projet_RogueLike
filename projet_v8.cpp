...
// g++ -Iextlib/pdcurses/include -Lextlib/pdcurses/lib projet_v7_4.cpp -lpdcurses -o projet.exe
...
void deplacement_joueur(string nom_fichier_map, int input, Joueur& Joueur) {
    if (input=='z') {
...
bool victoire(Joueur& joueur, string nom_fichier_map) {
    if (joueur.pos.x == 71 && joueur.pos.y == 6){
        return true;
    }
    return false;
}
...
int main() {
    ...
    while (input != 'a' &&  input != KEY_LEFT &&  input != KEY_RIGHT &&  input != KEY_UP  &&  input != KEY_DOWN) {
        ...
        if (input == 'p'){
            ...
            vector<Porte> list_door = initialisation_porte(nom_fichier);
        ...
        if (input == 'z' || input == 'q' || input == 's' || input == 'd') {
            ...
            if (ind_monstre != -1) {
                ...
                deplacement_joueur(nom_fichier, input, player);
