...
// g++ -Iextlib/pdcurses/include -Lextlib/pdcurses/lib projet_v8.cpp -lpdcurses -o projet.exe
...
void message_joueur(int message){
    ...
    if (message == 4){
        ecrire_string("Vous trouvez une epee vorpale, votre force augmente de 5.", 1, 24);
    }
    if (message == 5){
...
void deplacement_joueur(string nom_fichier_map, int input, Joueur& Joueur, Porte& porte) {
    if (input=='z') {
        if (est_mur(Joueur.pos.x, Joueur.pos.y-1, nom_fichier_map)  == false){
            if (est_porte(Joueur.pos.x, Joueur.pos.y-1, nom_fichier_map) == true){
                if (est_porte_ouverte(porte) == true){
                    Joueur.pos.y = Joueur.pos.y - 1;
                    ecrire_string(".", Joueur.pos.x, Joueur.pos.y+1);                   
                }
                else if (cle(Joueur)== true){
                    Joueur.pos.y = Joueur.pos.y - 1;
                    ecrire_string(".", Joueur.pos.x, Joueur.pos.y+1);
            ...
            else if(est_porte(Joueur.pos.x, Joueur.pos.y-1, nom_fichier_map) == false){
                Joueur.pos.y = Joueur.pos.y - 1;
                ecrire_string(".", Joueur.pos.x, Joueur.pos.y+1);  
        ...
        if (est_mur(Joueur.pos.x-1,Joueur.pos.y, nom_fichier_map) == false){
            if (est_porte(Joueur.pos.x-1,Joueur.pos.y, nom_fichier_map) == true){
                if (est_porte_ouverte(porte) == true){
                    Joueur.pos.x = Joueur.pos.x - 1;
                    ecrire_string(".", Joueur.pos.x+1, Joueur.pos.y);
                }
                else if (cle(Joueur) == true){
                    Joueur.pos.x = Joueur.pos.x - 1;
                    ecrire_string(".", Joueur.pos.x+1, Joueur.pos.y);
            ...
            else if (est_porte(Joueur.pos.x-1,Joueur.pos.y, nom_fichier_map) == false){
                Joueur.pos.x = Joueur.pos.x - 1;
                ecrire_string(".", Joueur.pos.x+1, Joueur.pos.y);
            }
        ...
        if (est_mur(Joueur.pos.x+1 ,Joueur.pos.y, nom_fichier_map) == false){
            ...
            else if (est_mur(Joueur.pos.x+1 ,Joueur.pos.y, nom_fichier_map) == false){
                Joueur.pos.x = Joueur.pos.x + 1;
                ecrire_string(".", Joueur.pos.x-1, Joueur.pos.y);
            }
    ...
    while (input != 'a' &&  input != KEY_LEFT &&  input != KEY_RIGHT &&  input != KEY_UP  &&  input != KEY_DOWN) {
        ...
        if (input == 'z' || input == 'q' || input == 's' || input == 'd') {
            ...
            if (ind_monstre != -1) {
                ...
                if (int_porte != -1){
                    deplacement_joueur(nom_fichier, input, player, list_door[int_porte]);
                }
                else if (int_porte == -1){
                    deplacement_joueur(nom_fichier, input, player, list_door[0]);
                }    
            ...
            if (victoire(player, nom_fichier) == true){
                fermer_map(nom_fichier);
                ecrire_string("Vous avez gagne", 37, 11);
            } 
